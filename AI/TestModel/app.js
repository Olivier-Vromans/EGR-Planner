...
function deleteNull(data) {
    ...
    const max = data.reduce(function (prev, current) {
        return (prev.CO2Emissions > current.CO2Emissions) ? prev : current
    }) //returns object

    //Lowest CO2 Emission
    const min = data.reduce(function (prev, curr) {
        return prev.CO2Emissions < curr.CO2Emissions ? prev : curr;
    });
    // Input minMax into index.html
    const minMax = document.getElementById("min/max")
    minMax.innerHTML = `Minimum C02 Emission: ${min.CO2Emissions}, Max CO2 Emission: ${max.CO2Emissions}`
